...
- **浏览器二进制**：有头时若系统装了 Google Chrome，默认用 Playwright `channel: "chrome"`，避免自带 `chrome-linux64/chrome` 崩窗。可用 `AUTO_REG_CHROME_PATH` 指定路径，或 `AUTO_REG_PLAYWRIGHT_CHANNEL=bundled` 强制自带 Chromium。
- **turnstilePatch（默认开启）**：打包了与 [JiuZX/Cursor-Register](https://github.com/JiuZX/Cursor-Register) / [TheFalloutOf76](https://github.com/TheFalloutOf76/CDP-bug-MouseEvent-.screenX-.screenY-patcher) / [Xewdy444](https://github.com/Xewdy444/CDP-bug-MouseEvent-.screenX-.screenY-patcher) 同源思路的 `script.js`。只修补 CDP 下 `screenX/screenY` 指纹，**不是验证码求解器**。
  - **默认只 `addInitScript`，不传 `--load-extension`**。用 `--load-extension` 挂该 MV3 时，Cloudflare 会报 **Incompatible browser extension**。
  - **`turnstileExtension`（默认 `false`）**：仅显式 `true` 且 headed 才 `--load-extension`（旧 vendor 行为，Cloudflare 会拦）。
  - **真实注册默认 headed**。遇到 Just a moment / 429 / 窗口崩溃会立刻失败，不会空等，也不会自动过 Cloudflare。
...
| `turnstileExtension` | **默认 `false`**：不要 `--load-extension` 挂 MV3；Cloudflare 会报 Incompatible browser extension |
| `timeoutMs` | 单步超时（毫秒，`> 0`） |
| `signupUrl` | 注册页地址 |
| `email` | 邮箱 / 收码配置，见下 |
...
| `AUTO_REG_CHROME_PATH` | 系统 Chrome 可执行文件路径 |
| `AUTO_REG_PLAYWRIGHT_CHANNEL` | `chrome` / `bundled` / `chromium` |
...
选项：
  ...
  --headed          浏览器有头（可见）运行。
  --headless        强制无头。真实注册默认 headed，便于人工过人机（不做自动过验证）。
  -h, --help        显示帮助。
```
