...
export interface AutoRegConfig {
    ...
    headed: boolean;
    /**
     * Inject the packaged turnstilePatch script.js via addInitScript
     * (CDP MouseEvent.screenX/Y fix from TheFalloutOf76 / Cursor-Register).
     * Default true. This is not a captcha solver — unresolved challenges
     * still fail or wait for a human. The MV3 is NOT loaded as a Chrome
     * extension unless {@link turnstileExtension} is also true.
     */
    turnstilePatch: boolean;
    /**
     * When true AND headed, also load resources/turnstilePatch via
     * `--load-extension`. Default false: Cloudflare Turnstile reports
     * "Incompatible browser extension" when that flag is set.
     */
    turnstileExtension: boolean;
    timeoutMs: number;
    signupUrl: string;
    email: EmailConfig;
